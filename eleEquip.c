...
void showMenuDevices()
{
	...
	while (1)
	{
		choice = 0;
		printf("-----Menu-----\n");
		printf("Select the connected device:\n");
		int i;
		printf("1. TV\n");
		printf("2. AR\n");
		printf("3. PC\n");
...
int runDevice(int deviceID, int mode)
{
	...
	printf("Successfully received the return result from connectMng\n");
	...
	if (strcmp(systemStatus, "OVER") == 0)
	{
		...
		{
			printf("System overload.\nThe device will run in %s mode.\n", equipStatus);
		}
	}
	else
	{
		printf("System has %s.\n The device is running in %s mode and %d W\n", systemStatus, equipStatus, voltage);
	}
