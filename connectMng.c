...
void powerSupply(command cmd)
{
    ...
    printf("Sending a message to elePowerCtrl...\n\n");
    ...
    if (strcmp(cmd.code, "STOP") == 0)
    {
        ...
        if(msgsnd(msgId1, &message1, sizeof(message1.mesg_text), 0) == -1) printf("Failed: Send a message to elePowerCtrl\n\n");
    }
    // nếu code là ON
    else
    {
    ...
    {
        printf("Success: Get the message from elePowerCtrl\n\n");
        ...
        printf("Success: Returns the result to the client\n\n");
    }
...
void sig_chld(int singno)
{
    ...
    while ((pid = waitpid(-1, &stat, WNOHANG)) > 0)
        printf("Kill the child process %d\n", pid);
    return;
}
...
int main()
{
    ...
    // connectMng
    // khởi tạo kết nối IP
    printf("Initiating IP connection...\n\n");
    if ((listenSock = socket(AF_INET, SOCK_STREAM, 0)) < 0)
    {
        printf("An error occurred while initializing socket\n");
        exit(1);
    }
    ...
    {
        printf("An error occurred while binding the address\n");
        exit(2);
    }
